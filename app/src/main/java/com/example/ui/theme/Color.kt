package com.example.ui.theme

import androidx.compose.ui.graphics.Color

val Purple80 = Color(0xFFD0BCFF)
val PurpleGrey80 = Color(0xFFCCC2DC)
val Pink80 = Color(0xFFEFB8C8)

val Purple40 = Color(0xFF6650a4)
val PurpleGrey40 = Color(0xFF625b71)
val Pink40 = Color(0xFF7D5260)

// Sleek Interface Theme Palette
val SleekPrimary = Color(0xFF6750A4)
val SleekOnPrimary = Color(0xFFFFFFFF)
val SleekPrimaryContainer = Color(0xFFE8DEF8)
val SleekOnPrimaryContainer = Color(0xFF21005D)
val SleekSecondary = Color(0xFF49454F)
val SleekOnSecondary = Color(0xFFFFFFFF)
val SleekSecondaryContainer = Color(0xFFF3EEF4)
val SleekBackground = Color(0xFFFDF8F6)
val SleekOnBackground = Color(0xFF1D1B1E)
val SleekSurface = Color(0xFFFFFFFF)
val SleekOnSurface = Color(0xFF1D1B1E)
val SleekSurfaceVariant = Color(0xFFF3EEF4)
val SleekOnSurfaceVariant = Color(0xFF49454F)
val SleekOutlineVariant = Color(0xFFEADDFF)
